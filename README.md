...
## 썸네일 처리 변경: video.poster 사용으로 통합

홈화면의 비디오 카드(Item 컴포넌트)에서 별도의 <img> 썸네일을 렌더하던 기존 구현을 제거하고, HTML <video> 요소의 poster 속성을 사용하도록 변경했습니다. 변경 파일: `app/page.js` (Item 컴포넌트).

변경 이유

- DOM 요소 수 감소 및 렌더 복잡성 축소
- 브라우저의 네이티브 poster 지원을 이용해 초기 프레임(썸네일)을 표시
- 기존의 캔버스 기반 프레임 캡처나 복잡한 썸네일 폴백 로직을 제거하여 모바일/브라우저 호환성 향상

동작 요약

- poster에 기존 썸네일 URL(`video.thumbs.url3`)을 전달합니다.
- 초기에는 poster가 보이며, 사용자가 재생하거나 IntersectionObserver에 의해 로드가 트리거되면 src에 스트리밍 URL이 설정되어 비디오가 재생됩니다.
- preload="none"을 유지하여 불필요한 네트워크 사용을 줄입니다.
- 로딩 중에는 기존의 스피너(로더)가 여전히 표시됩니다.

주의사항

- poster 이미지가 외부 도메인에 있을 경우 CORS로 인해 일부 브라우저에서 로드되지 않을 수 있습니다. 가능한 경우 썸네일을 동일 출처 또는 CORS 허용된 호스트로 제공하세요.
- poster는 브라우저 구현에 따라 다르게 동작할 수 있으므로 디자인(가로/세로 비율, 높이 고정 등)을 CSS로 명확히 지정하는 것이 좋습니다.
- poster에 사용되는 임시 링크도 만료되는 경우가 있으므로, 서버에서 영구 썸네일 URL을 제공하거나 배경 작업으로 미리 생성해두는 것을 권장합니다.

권장 작업

- poster 실패(fallback) 처리를 위해 로컬 아이콘 또는 SVG를 대체 이미지로 설정하는 로직 추가.
- 서버 측에서 썸네일을 빌드 시점이나 백그라운드 작업으로 생성하면 CORS/만료 문제를 완벽히 제거할 수 있습니다.
- 사용자 기기별 네트워크 조건에 따라 poster 해상도를 조정하거나 WebP/AVIF 같은 효율적 이미지 포맷을 제공하면 UX가 개선됩니다.
