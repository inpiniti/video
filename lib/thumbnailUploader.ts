...
export async function ensureFfmpeg() {
  ...
  if (!createFFmpegFn && !FFmpegClass) {
    throw new Error('Neither createFFmpeg nor FFmpeg class export found. Last error: ' + (lastErr ? String(lastErr) : 'unknown'));
  }
  if (createFFmpegFn) {
  ...
  ffmpegInstance = {
    ...
    load: async () => { if (!inst.loaded) await inst.load(); },
...
async function captureViaVideoElement(url: string): Promise<Blob> {
  return new Promise<Blob>((resolve, reject) => {
    ...
    video.addEventListener('error', () => fail(new Error('video element error loading resource')));
    video.addEventListener('loadeddata', () => {
      try {
        // Try to seek to 1s
        const target = Math.min(1, (video.duration && isFinite(video.duration)) ? video.duration - 0.05 : 1);
        const proceed = () => {
          try {
    ...
    timeoutId = window.setTimeout(() => fail(new Error('video load timeout')), 10000);
    video.src = url;
    video.load();
  });
}
...
export async function uploadToDropbox(blob: Blob, baseName: string): Promise<string> {
  const token = await getDropboxToken();
  if (!token) throw new Error("Dropbox token missing. Perform OAuth first.");
  const path = `/thumbnails/${Date.now()}_${baseName}.jpg`;
  ...
  if (!uploadRes.ok) {
    ...
    if (uploadRes.status === 401 && /expired_access_token/.test(detail)) {
      try { localStorage.removeItem('DROPBOX_ACCESS_TOKEN'); } catch {/* ignore */}
      throw new Error("Dropbox token expired. 다시 'Dropbox 연결' 버튼으로 재인증하세요. Raw: " + detail);
    }
    throw new Error("Dropbox upload failed: " + uploadRes.status + (detail ? ' ' + detail : ''));
  }
  // Create shared link
  ...
  if (!linkRes.ok) {
    let detail = '';
    try { detail = await linkRes.text(); } catch { /* ignore */ }
    throw new Error("Dropbox link create failed: " + linkRes.status + (detail ? ' ' + detail : ''));
  }
...
export async function generateAndUploadThumbnail(videoUrl: string): Promise<ThumbnailResult> {
  let blob: Blob;
  ...
  try {
    ...
    try {
      ...
      throw new Error('FFmpeg & canvas fallback failed: ' + (inner instanceof Error ? inner.message : String(inner)) + ' (original: ' + (e instanceof Error ? e.message : String(e)) + ')');
    }
  }
...
export async function saveThumbnailToSupabase(id: number, thumbnailUrl: string) {
  if (!hasSupabase() || !supabase) throw new Error("Supabase not configured");
  const { error } = await supabase.from("videos").update({ thumbnail: thumbnailUrl }).eq("id", id);
